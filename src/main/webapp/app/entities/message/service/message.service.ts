...
type RestOf<T extends IMessage | NewMessage> = Omit<T, 'chatDate'> & {
...
export class MessageService {
  ...
  protected convertDateFromClient<T extends IMessage | NewMessage | PartialUpdateMessage>(message: T): RestOf<T> {
    return {
      ...message,
      chatDate: message.chatDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restMessage: RestMessage): IMessage {
    return {
      ...restMessage,
      chatDate: restMessage.chatDate ? dayjs(restMessage.chatDate) : undefined,
    };
  }
